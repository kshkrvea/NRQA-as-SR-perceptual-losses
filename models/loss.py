...
from .select_feature_extractor import select_FE
...
class FeatureExtractor(nn.Module):
    def __init__(self, feature_layer=[2,7,16,25,34], use_input_norm=True, use_range_norm=False, model='vgg', device='cpu'):
        super(FeatureExtractor, self).__init__()
        '''
        use_input_norm: If True, x: [0, 1] --> (x - mean) / std
        use_range_norm: If True, x: [0, 1] --> x: [-1, 1]
        '''
        model = select_FE(model, device=device)
        ...
        if self.list_outputs:
            ...
            for i in range(len(feature_layer)-1):
                self.features.add_module('child'+str(i), nn.Sequential(*list(model.children())[(feature_layer[i]+1):(feature_layer[i+1]+1)]))
        else:
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self, 
        ...
        self.vgg = FeatureExtractor(feature_layer=feature_layer, 
...
class LpipsLoss(nn.Module):
    ...
    def __init__(self, **args):
        ...
        import lpips
        self.loss_fn = lpips.LPIPS(**args)

    def forward(self, x, y):
        if len(x.shape) == 5:            
            b, t, h, w, c = x.shape
            x = torch.reshape(x, (b*t, h, w, c))
            y = torch.reshape(y, (b*t, h, w, c))
