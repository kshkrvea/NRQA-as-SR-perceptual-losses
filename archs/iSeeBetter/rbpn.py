...
class Net(nn.Module):
    ...
    def forward(self, x, neigbor, flow):
        ...
        feat_frame=[]
        for j in range(neigbor.size()[1]):#CHANGED
            feat_frame.append(self.feat1(torch.cat((x, neigbor[:, j], flow[:, j]),1))) #CHANGED
...
class GeneratorLoss(nn.Module):
    ...
    def forward(self, out_labels, hr_est, hr_img, idx):
        ...
        return image_loss + 2e-8 * tv_loss + 0.006 * perception_loss + 1e-3 * adversarial_loss #1e-3 * adv
